"""
Core tracing functionality for Bedrock Agent LangSmith integration.
"""
import json
import logging
...
def get_tracer(tracer_name="bedrock-agent-langsmith"):
...
def format_token_usage(input_tokens, output_tokens):
    """Format token usage for LangSmith in the expected format"""
...
def flush_telemetry():
    """Force flush all pending telemetry data to LangSmith"""
    try:
        ...
        if success:
            logger.info("🟢 Telemetry data flushed successfully to LangSmith")
        else:
            logger.warning(
