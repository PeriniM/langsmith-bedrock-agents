    ...
    with tracer.start_as_current_span(
        ...
        with tracer.start_as_current_span(
            ...
            if "metadata" in model_output:
                ...
                assessment_span.set_attribute("metadata", json.dumps(metadata, cls=DateTimeEncoder))
    ...
    if "modelInvocationOutput" in component_trace:
        ...
        with tracer.start_as_current_span(
            ...
            with tracer.start_as_current_span(
                ...
                if "metadata" in model_output:
                    metadata = model_output["metadata"]
                    output_span.set_attribute("metadata", json.dumps(metadata, cls=DateTimeEncoder))
...
def handle_knowledge_base(trace_data: Dict[str, Any], parent_span):
    ...
    if (
        ...
        set_span_attributes(
            kb_span,
            {
                "metadata": json.dumps(kb_metadata, cls=DateTimeEncoder),
    ...
    if (
        ...
        with tracer.start_as_current_span(
            ...
            if "metadata" in final_response:
                final_response_span.set_attribute(
                    "response.metadata", json.dumps(final_response["metadata"], cls=DateTimeEncoder)
                )
    ...
    with tracer.start_as_current_span(
        ...
        if obs:
            ...
            if "ask_user_metadata" in obs:
                user_input_span.set_attribute(
                    "ask_user.metadata", json.dumps(obs["ask_user_metadata"], cls=DateTimeEncoder)
                )
        ...
        for idx, this_file in enumerate(files_list):
            ...
            if "metadata" in this_file:
                file_span.set_attribute(
                    f"file.{idx}.metadata",
                    json.dumps(this_file["metadata"], cls=DateTimeEncoder)
                )
